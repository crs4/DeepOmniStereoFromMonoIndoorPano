...
class EdgeDetector(nn.Module):
  ...
  def forward(self, im):
    if(self.use_cuda):
        self.filter_horz.weight.data = torch.cuda.FloatTensor([[[[-1, -1, -1], [0, 0, 0], [1, 1, 1]]]])
        self.filter_vert.weight.data = torch.cuda.FloatTensor([[[[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]]]])
    else:
        self.filter_horz.weight.data = torch.Tensor([[[[-1, -1, -1], [0, 0, 0], [1, 1, 1]]]])
        self.filter_vert.weight.data = torch.Tensor([[[[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]]]])
