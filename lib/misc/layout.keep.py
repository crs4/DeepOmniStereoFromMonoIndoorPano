...
class D2L(nn.Module):
    ...
    def get_src_layout(self, src_depth, src_layout_seg):
        ...
        for i in range(B):
            ...
            m_contour = self.contour_from_cmask(layout_mask.cpu(), epsilon_b=0.01)####NB. numpy-cpu needed by opencv 
        ...
        return torch.Tensor(LS).to(p_max.device), p_max, p_min

    def translated_edges_from_layout(self, src_layout, p_max, p_min, W, H, x_c):####from numpy arrays
        B,_,_,_ = src_layout.size()
                                       
        LE = []
                
        for i in range(B):
            equi_pts = self.contour_pts2equi_layout(src_layout[i:i+1], W, H, p_max, p_min, translation = x_c)

            le = get_layout_edges(equi_pts.unsqueeze(0), H, W)
    ...
    def contour_pts2equi_layout( self, c_pts, W, H, c_dist, f_dist, translation):
       
        xy = self.atl_pts2xy(c_pts, c_dist, f_dist)
                        
        ####apply translation
        ##print(xy.shape, xy.device)
              
        xy[:,:1]  -= translation[:,:1]
        xy[:,1:2] += translation[:,1:2]

        off = (translation[:,2:].squeeze(0))[0]
        ...
        ####default: cartesian order
        c_cor = xy2coor(np.array(xy.cpu()),  c_dist.cpu(), W, H)#########FIXME - replace with torch
        f_cor = xy2coor(np.array(xy.cpu()), -f_dist.cpu(), W, H) ####based on the ceiling shape
        ...
        equi_coords = torch.Tensor(equi_coords)

        print(equi_coords.shape)
