...
class depth_separable_conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, stride, padding, dilation):
        ...
        ##groups= in_channels, each input channel is convolved with its own set of filters (of size \frac{\text{out\_channels}}{\text{in\_channels}} 
        ###depthwise conv - 
...
class GatedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride = 1, padding = 0, dilation = 1, pad_type = 'replicate', 
                 activation = 'elu', norm = 'none', sc=False, sn=False, return_mask = False):
        ...
        # Initialize the convolution layers - NB modifcation of the orginal gaednet: here was spectral norm option
        if sc:
            self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding = 0, dilation = dilation)
            self.mask_conv2d = sc_conv(in_channels, out_channels, kernel_size, stride, padding = 0, dilation = dilation) ###single channel simple conv2d
        else:
            self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding = 0, dilation = dilation)
            #self.mask_conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=0, dilation=dilation)
            self.mask_conv2d = depth_separable_conv(in_channels, out_channels, kernel_size, stride, padding = 0,dilation=dilation) #### CRA approach - Light-Weight Gated Convolutions
    ...
    def forward(self, x_in):
        ...
        ##print('conv mask',conv.shape,mask.shape)
